...
if args.pickled_data:
    ...
    path = "data/atomic/processed/generation/{}.pickle".format(utils.make_name_string(opt.data))
...
with torch.no_grad():
    for idx in tqdm(total):
        ...
        if args.comet_completion:
            XMB = batch["sequences"][:, :batch['len_before_lastobj'][0]]
            #Ref = batch["sequences"][:, context_size_event + 1:]
            MMB = batch["attention_mask"][:, :batch['len_before_lastobj'][0]]
            subject_input = ' '.join([data_loader.vocab_decoder[item] for item in XMB[0].tolist()])
            gold_obj_toks = [item for item in batch["sequences"][:, batch['len_before_lastobj'][0]:][0].tolist() if not item==0]
            gold_object = ' '.join([data_loader.vocab_decoder[item] for item in gold_obj_toks]) 
            print(subject_input)
            print(gold_object)
        else:
            #XMB = batch["sequences"][:, :5]
            subject_input = ""
            ...
            #Ref = batch["sequences"][:, context_size_event + 1:]
            #MMB = batch["attention_mask"][:, :5]
            MMB = torch.ones_like(XMB).float().cuda()
            #init = "<START>"
